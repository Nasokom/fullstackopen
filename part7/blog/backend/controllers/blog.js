...
blogRouter.post("/", async (request, response) => {
  if (!request.body.title || !request.body.url) {
    return response
      .status(400)
      .send({ error: "Missing title || url properties" });
  }
...
blogRouter.delete("/:id", async (request, response) => {
  ...
  if (!x) {
    return response
      .status(404)
      .send({ error: `No item found with the id: ${id}` })
      .end();
  }
...
blogRouter.put("/:id", async (request, response) => {
  ...
  if (!body.url || !body.title || !body.author) {
    return response
      .status(400)
      .json({ error: "Bad Request missing properties" });
  }
  ...
  if (!target) {
    return response
      .status(404)
      .json({ error: `No blog post found with the id ${id}` });
  }
