...
test("POST user missing content => 400 && error msg", async () => {
  const { body } = await api.post(url).send({}).expect(400);
...
test("POST duplicate user => 400 && error msg", async () => {
  ...
  const { body } = await api.post(url).send(duplicateUser).expect(400);
...
test("POST valid content => 201 && {name,username,id}", async () => {
  ...
  //no password || hash
  assert(
    !body.hasOwnProperty("passwordHash") || !body.hasOwnProperty("password"),
  );
});
